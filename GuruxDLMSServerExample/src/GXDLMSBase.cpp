  ...
  while (server->IsConnected()) {
    ...
    if (server->IsConnected()) {
      ...
      while (server->IsConnected()) {
        ...
        if ((ret = recv(socket, (char *)bb.GetData() + bb.GetSize(),
        ...
        if (reply.GetSize() != 0) {
          ...
          if (send(socket, (const char *)reply.GetData(),
                   reply.GetSize() - reply.GetPosition(), 0) == -1) {
...
int CGXDLMSBase::GetSocket() { return (int)m_ServerSocket; }
...
int CGXDLMSBase::StartServer(int port) {
  ...
  int fFlag = 1;
  if (setsockopt(m_ServerSocket, SOL_SOCKET, SO_REUSEADDR, (char *)&fFlag,
                 sizeof(fFlag)) == -1) {
    // setsockopt.
    return -1;
  }
...
#else
  ret =
      pthread_create(&m_ReceiverThread, NULL, UnixListenerThread, (void *)this);
#endif
  return ret;
}
...
// The first three octets uniquely identify the manufacturer of the device and
// it corresponds to the manufacturer's identification in IEC 62056-21. The
// following 13 octets are assigned by the manufacturer.
// The manufacturer is responsible for guaranteeing the uniqueness of these
// octets.
CGXDLMSData *AddLogicalDeviceName(CGXDLMSObjectCollection &items,
                                  unsigned long sn) {
  char buff[17];
#if defined(_WIN32) || defined(_WIN64) // Windows
...
void AddActivityCalendar(CGXDLMSObjectCollection &items) {
  ...
  CGXDateTime startSummerTime(-1, 3, 31, 0, 0, 0, 0);
  pActivity->GetSeasonProfileActive().push_back(
      new CGXDLMSSeasonProfile("Summer time", startSummerTime, ""));
  pActivity->GetWeekProfileTableActive().push_back(
      new CGXDLMSWeekProfile("Monday", 1, 1, 1, 1, 1, 1, 1));
  CGXDLMSDayProfile *aDp = new CGXDLMSDayProfile();
  aDp->SetDayId(1);
  CGXDateTime now = CGXDateTime::Now();
  CGXTime time = now;
  aDp->GetDaySchedules().push_back(
  ...
  CGXDateTime startWinterTime(-1, 10, 30, 0, 0, 0, 0);
  pActivity->GetSeasonProfilePassive().push_back(
      new CGXDLMSSeasonProfile("Winter time", startWinterTime, ""));
  pActivity->GetWeekProfileTablePassive().push_back(
      new CGXDLMSWeekProfile("Tuesday", 1, 1, 1, 1, 1, 1, 1));

  CGXDLMSDayProfile *passive = new CGXDLMSDayProfile();
  passive->SetDayId(1);
  passive->GetDaySchedules().push_back(
      new CGXDLMSDayProfileAction(time, "0.0.1.0.0.255", 1));
 ...
 */
void AddRegisterMonitor(CGXDLMSObjectCollection &items,
                        CGXDLMSRegister *pRegister) {
...
int CGXDLMSBase::Init(int port, GX_TRACE_LEVEL trace) {
  ...
  CGXDLMSProfileGeneric *profileGeneric =
      new CGXDLMSProfileGeneric("1.0.99.1.0.255");
  ...
  profileGeneric->GetCaptureObjects().push_back(
      std::pair<CGXDLMSObject *, CGXDLMSCaptureObject *>(pClock, capture));
  // Set saved attribute index.
  capture = new CGXDLMSCaptureObject(2, 0);
  profileGeneric->GetCaptureObjects().push_back(
      std::pair<CGXDLMSObject *, CGXDLMSCaptureObject *>(pRegister, capture));
  ...
  // the Push listener.
  pPush->GetPushObjectList().push_back(
      std::pair<CGXDLMSObject *, CGXDLMSCaptureObject>(
          pPush, CGXDLMSCaptureObject(2, 0)));
  // Add logical device name.
  pPush->GetPushObjectList().push_back(
      std::pair<CGXDLMSObject *, CGXDLMSCaptureObject>(
          ldn, CGXDLMSCaptureObject(2, 0)));
...
CGXDLMSObject *CGXDLMSBase::FindObject(DLMS_OBJECT_TYPE objectType, int sn,
                                       std::string &ln) {
  return NULL;
}
 ...
 */
void GetProfileGenericDataByEntry(CGXDLMSProfileGeneric *p, long index,
                                  long count) {
  int len, month = 0, day = 0, year = 0, hour = 0, minute = 0, second = 0,
           value = 0;
  ...
  if (count != 0) {
    ...
    if (f != NULL) {
#if defined(_WIN32) || defined(_WIN64) // Windows
      while ((len = fscanf_s(f, "%d/%d/%d %d:%d:%d;%d", &month, &day, &year,
                             &hour, &minute, &second, &value)) != -1)
#else
      while ((len = fscanf(f, "%d/%d/%d %d:%d:%d;%d", &month, &day, &year,
                           &hour, &minute, &second, &value)) != -1)
#endif
      {
        // Skip row
        if (index > 0) {
          ...
          CGXDateTime tm(2000 + year, month, day, hour, minute, second, 0,
                         0x8000);
...
void GetProfileGenericDataByRange(CGXDLMSValueEventArg *e) {
  ...
  if (f != NULL) {
    ...
    while ((len = fscanf_s(f, "%d/%d/%d %d:%d:%d;%d", &month, &day, &year,
                           &hour, &minute, &second, &value)) != -1)
#else
    while ((len = fscanf(f, "%d/%d/%d %d:%d:%d;%d", &month, &day, &year, &hour,
                         &minute, &second, &value)) != -1)
#endif
    {
...
void CGXDLMSBase::PreRead(std::vector<CGXDLMSValueEventArg *> &args) {
  ...
  std::string ln;
  for (std::vector<CGXDLMSValueEventArg *>::iterator it = args.begin();
       it != args.end(); ++it) {
    ...
    if (type == DLMS_OBJECT_TYPE_PROFILE_GENERIC) {
      ...
      if (index == 7) {
        ...
        if ((*it)->GetRowEndIndex() - (*it)->GetRowBeginIndex() >
            (*it)->GetRowToPdu()) {
    ...
    if (type == DLMS_OBJECT_TYPE_CLOCK && index == 2) {
      ...
      if (tp == DLMS_DATA_TYPE_INT8 || tp == DLMS_DATA_TYPE_INT16 ||
          tp == DLMS_DATA_TYPE_INT32 || tp == DLMS_DATA_TYPE_INT64 ||
          tp == DLMS_DATA_TYPE_UINT8 || tp == DLMS_DATA_TYPE_UINT16 ||
          tp == DLMS_DATA_TYPE_UINT32 || tp == DLMS_DATA_TYPE_UINT64) {
        // Initialize random seed.
        srand((unsigned int)time(NULL));
        value = rand() % 100 + 1;
        value.vt = tp;
...
void HandleImageTransfer(CGXDLMSValueEventArg *e) {
  ...
  if (e->GetIndex() == 1) {
    ...
    *p = '\0';
#if defined(_WIN32) || defined(_WIN64) // If Windows
    strncat_s(IMAGEFILE, sizeof(IMAGEFILE),
              (char *)e->GetParameters().Arr[0].byteArr,
              (int)e->GetParameters().Arr[0].GetSize());
    strcat_s(IMAGEFILE, ".bin");
#else
    strncat(IMAGEFILE, (char *)e->GetParameters().Arr[0].byteArr,
            (int)e->GetParameters().Arr[0].GetSize());
    strcat(IMAGEFILE, ".bin");
#endif
#if defined(_WIN32) || defined(_WIN64) ||                                      \
    defined(__linux__) // If Windows or Linux
  ...
  // Transfers one block of the Image to the server
    ...
    int ret = fwrite(e->GetParameters().Arr[1].byteArr, 1,
                     (int)e->GetParameters().Arr[1].GetSize(), f);
    fclose(f);
...
int CGXDLMSBase::SendPush(CGXDLMSPushSetup *target) {
  ...
  if ((ret = GeneratePushSetupMessages(NULL, target, reply)) == 0) {
    ...
    for (std::vector<CGXByteBuffer>::iterator it = reply.begin();
         it != reply.end(); ++it) {
      if ((ret = send(socket, (const char *)it->GetData(), it->GetSize(), 0)) ==
          -1) {
        break;
      }
    }
...
void Capture(CGXDLMSProfileGeneric *pg) {
  ...
  f = fopen(DATAFILE, "a");
#endif
  for (std::vector<std::pair<CGXDLMSObject *, CGXDLMSCaptureObject *>>::iterator
           it = pg->GetCaptureObjects().begin();
    ...
    if (it->first->GetObjectType() == DLMS_OBJECT_TYPE_CLOCK &&
        it->second->GetAttributeIndex() == 2) {
...
bool CGXDLMSBase::IsTarget(unsigned long int serverAddress,
                           unsigned long clientAddress) {
  return true;
}

DLMS_SOURCE_DIAGNOSTIC
CGXDLMSBase::ValidateAuthentication(DLMS_AUTHENTICATION authentication,
  ...
  if (authentication == DLMS_AUTHENTICATION_LOW) {
    ...
    if (expected.GetSize() == password.GetSize() &&
        expected.Compare(password.GetData(), password.GetSize())) {
...
CGXDLMSBase::GetMethodAccess(CGXDLMSValueEventArg *arg) {
...
//
/////////////////////////////////////////////////////////////////////////////
void CGXDLMSBase::Connected(CGXDLMSConnectionEventArgs &connectionInfo) {
  if (m_Trace > GX_TRACE_LEVEL_WARNING) {
    printf("Connected.\r\n");
  }
}
...
//
/////////////////////////////////////////////////////////////////////////////
void CGXDLMSBase::Disconnected(CGXDLMSConnectionEventArgs &connectionInfo) {
  if (m_Trace > GX_TRACE_LEVEL_WARNING) {
    printf("Disconnected.\r\n");
  }
}
...
void CGXDLMSBase::PostGet(std::vector<CGXDLMSValueEventArg *> &args) {}